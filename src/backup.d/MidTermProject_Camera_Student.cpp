...
int main(int argc, const char *argv[])
{
    ...
    const char *keys =
	"{help h || help messge}" 
	"{detector detector |foo| detector type}" 
	;
    
    cv::CommandLineParser parser (argc, argv , keys);
    string detectorType  = parser.get<string>("detector");
    cout << " Input detector = " << detectorType << endl;
	...
	///  string-based selection based on detectorType
	    ...
	    cout << keepers.size() << " number of keeperts\n";	
	    keypoints.swap (keepers);
	}
        //// EOF STUDENT ASSIGNMENT
